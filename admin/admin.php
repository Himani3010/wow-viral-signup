...
<?php
function wow_signup_admin_menu() {	
add_menu_page('Wow Viral Signups Pro', __( "Wow<br/> Viral Signups Pro", "wow-marketings"), 'manage_options', 'wow-viral-signups-pro', 'wow_signup', 'dashicons-groups', null); 
add_action('admin_print_styles', 'wow_script_style');
}
add_action('admin_menu', 'wow_signup_admin_menu', 999);

function wow_signup() {
	global $wow_plugin;	
	$wow_plugin = true;
	include_once( 'partials/signup.php' );	
	wp_enqueue_style('signup-prevew', plugin_dir_url(__FILE__) . 'preview/preview.css');
	wp_enqueue_script('jquery-signup-preview', plugin_dir_url(__FILE__) . 'preview/preview.js', array('jquery'));
	wp_enqueue_script( 'wow-signup', plugin_dir_url(__FILE__) . 'js/signup.js', array( 'jquery' ));
